...
int main() {
    ...
    // CPU 参考
    printf("--- CPU 计算 ---\n");
    cudaEvent_t cpuStart, cpuEnd;
    cudaEventCreate(&cpuStart); cudaEventCreate(&cpuEnd);
    cudaEventRecord(cpuStart);
    conv1dCPU(h_in, h_ref, n);
    cudaEventRecord(cpuEnd);
    // CPU 计时用 CUDA event 不准, 用粗略方式
    printf("  CPU 计算完成\n\n");
    ...
    printf("--- 朴素卷积 (全局内存) ---\n");
    {
        ...
        printf("  耗时: %.3f ms (100次平均)\n", ms / 100);
        printf("  最大误差: %e\n", maxErr);
        printf("  有效带宽: %.2f GB/s\n", (2.0f * size) / (ms / 100 * 1e6));
